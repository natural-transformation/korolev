...
class ClientEventListenerSpec extends AnyFlatSpec with Matchers {
  "Korolev client listener" should "walk up to a parent node with vId" in {
    val path = Paths.get("modules/korolev/src/main/es6/korolev.js")
  ...
  it should "open long-polling early to attach listeners" in {
    val path = Paths.get("modules/korolev/src/main/es6/connection.js")
    val source = new String(Files.readAllBytes(path), "UTF-8")
  ...
  it should "report extractProperty failures in the bridge" in {
    val path = Paths.get("modules/korolev/src/main/es6/bridge.js")
    val source = new String(Files.readAllBytes(path), "UTF-8")
  ...
  it should "pass config into Connection" in {
    val path = Paths.get("modules/korolev/src/main/es6/launcher.js")
    val source = new String(Files.readAllBytes(path), "UTF-8")
    source.contains("new Connection") shouldBe true
    source.contains("window.location,\n    config") shouldBe true
  }

  it should "mark Korolev ready on connection ready" in {
    val path = Paths.get("modules/korolev/src/main/es6/launcher.js")
    val source = new String(Files.readAllBytes(path), "UTF-8")
    source.contains("addEventListener('ready'") shouldBe true
    source.contains("KorolevReady") shouldBe true
    source.contains("Korolev']['ready'] = true") shouldBe true
  }
}
