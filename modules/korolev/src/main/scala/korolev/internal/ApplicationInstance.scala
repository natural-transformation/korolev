...
final class ApplicationInstance[
  ...
  private def onState(maybeRenderCallback: Seq[Effect.Promise[Unit]]): F[Unit] =
    for {
      ...
      _ <- Effect[F].delay {
             ...
             renderContext.swap()
             // Levsha's diff algorithm mutates its internal IdBuilder. Reset it before each render
             ...
             // Levsha's DiffRenderContext expects the freshly-rendered document to be finalized
  ...
  def initialize()(implicit ec: ExecutionContext): F[Unit] = {
    ...
    // - Diffing against an empty rhs buffer can put Levsha's internal IdBuilder into an invalid state,
    //   which breaks subsequent renders (see Issue14Spec after the Levsha upgrade).
